...
//     http://fsharp.org
//     https://docs.microsoft.com/ja-jp/dotnet/articles/fsharp/
//
// このチュートリアルをドキュメント形式で表示するには、次を参照してください:
//     https://docs.microsoft.com/ja-jp/dotnet/articles/fsharp/tour
//
...
///
/// 詳細については、次を参照してください: 
///     https://docs.microsoft.com/ja-jp/dotnet/articles/fsharp/language-reference/primitive-types
///     および
///     https://docs.microsoft.com/ja-jp/dotnet/articles/fsharp/language-reference/symbol-and-operator-reference/boolean-operators
module Booleans =
...
module PipelinesAndComposition =
    ...
    /// 'squareOddValuesAndAddOneNested' のような関数を入れ子にするよりもはるかに読みやすくすることができます
...
module Arrays =
    ...
    /// これは空の配列です。構文はリストの構文と似ていますが、代わりに `[| ... |]` を使用することに注意してください。
    let array1 = [| |]
...
module RecordTypes = 
    ...
    /// この例では、レコード値を処理する関数を記述する方法を示します。
...
/// DU に格納されるデータは、複数の個別の値のいずれかになります。
///
